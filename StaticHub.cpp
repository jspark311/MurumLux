...
volatile void StaticHub::log(const char *fxn_name, int severity, const char *str, ...) {
  ...
  va_end(marker);
  Serial.print((char*)  log_buffer.string());
  log_buffer.clear();
}
...
void StaticHub::init_RNG(void) volatile {
  randomSeed(time(NULL));          // Seed the PRNG...
}
...
StaticHub::StaticHub() {
  ...
  usb_string_waiting   = false;

  // Scheduler PIDs that will be heavilly used...
  pid_log_moderator   = 0;  // Moderate the logs running into the USB line.
  pid_profiler_report = 0;  // Internal testing. Allows periodic profiler dumping.
  pid_prog_run_delay  = 0;  // Give the programmer a chance to stop system init before it get unmanagable.
  rtc_startup_state = MANUVR_RTC_STARTUP_UNINITED;  // This is how we know what state we found the RTC in.
}
